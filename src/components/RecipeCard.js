...
return (
  <div 
    className="relative flex flex-col items-center justify-between w-full max-w-sm bg-earth text-white border border-gray-300 rounded-lg shadow-md hover:shadow-lg transition-shadow p-4"
    onClick={() => router.push(`/recipe/${recipe.id}`)}
  >
    {/* Título de la receta */}
    <h1 className="text-xl font-bold text-center mb-4">{recipe.titulo}</h1>
    ...
    <div className="flex flex-col sm:flex-row gap-2 w-full justify-center mt-4">
      <button 
        className="flex-1 bg-mint hover:bg-green-300 px-4 py-2 text-black font-bold rounded transition-colors"
        onClick={(e) => {
          e.stopPropagation();
          router.push(`/editar/${recipe.id}`);
        }}
      ...
      <button 
        className="flex-1 bg-beige hover:bg-orange px-4 py-2 text-black font-bold rounded transition-colors"
        onClick={(e) => {
          e.stopPropagation();
          const accept = window.confirm("¿Estás seguro de que deseas eliminar esta receta?");
          if (accept) {
